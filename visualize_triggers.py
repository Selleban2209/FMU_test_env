...
def setup_fmu3_slave(fmu_path, unzip_directory="unzip_dir"):
    ...
    while current_time < stop_time - step_size:
        ...
        height = fmu3_slave.getFloat64([vrs['h']])[0]
        velocity = fmu3_slave.getFloat64([vrs['v']])[0]
        if process_triggers(evaluation_output) == "Ball close to ground" and not flip:
            flip = True
           # fmu3_slave.setString([vrs['rtlola_spec']], ["specifications/new_ball_spec.lola", "h", "v"])
            fmu3_slave.setFloat64([vrs['v']], [velocity+2])
            trigger_times.append(current_time)
            trigger_heights.append(height)
        #print("Current height:", h_value)
        results.append((current_time, height))
       
        current_time += step_size
    
    
    
    # Step 5: Terminate the FMU




    result = np.array(results, dtype=np.dtype([('time', np.float64), ('h', np.float64)]))

    # Extract trigger times and heights
    trigger_times_np = np.array(trigger_times, dtype=np.dtype([('time', np.float64), ('h', np.float64)]))
    ...
    # Add markers for trigger points
    fig.add_trace(go.Scatter(
        x=trigger_times,
        y=trigger_heights,
        mode='markers',
